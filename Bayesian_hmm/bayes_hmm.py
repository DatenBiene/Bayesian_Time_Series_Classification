...
class bayesian_hmm:
    ...
    def likelihood(self, new_series):
        """ Compute the likelihood that a new series was generated by trained self.bayesian_hmm using the Forward algorithm.
            Should consider log form or other approach to handle underflow (should not happen with reasonnably short sequences).
            -----------
            Parameters :
            new_series : list like object of floats values and arbitrary length
            -----------
            Returns : 
            float64 : P(new_series | model)
        """
        try: 
            self.posterior_P 
        except:
        ...
        #Forward algo
        T = len(new_series)
        N = self.posterior_P.shape[0]
        alpha = np.zeros((T, N))
        alpha[0] = (1./N) * B[:, 0]
        for t in range(1, T):
            alpha[t] = alpha[t-1].dot(self.posterior_P) * B[:, t]
